...
class SparseBasicBlock(paddle.nn.Layer):
    ...
    def __init__(
        ...
        self.conv1 = conv3x3(
            in_channels, out_channels, stride, indice_key=indice_key, bias_attr=bias_attr)
        self.bn1 = nn.BatchNorm(out_channels, epsilon=1e-3, momentum=0.01)
        self.relu = nn.ReLU()
        self.conv2 = conv3x3(out_channels, out_channels, indice_key=indice_key, bias_attr=bias_attr)
...
class SparseResNet3D(paddle.nn.Layer):
    ...
    def forward(self, voxel_features, coors, batch_size):
        ...
        np.save(prefix + "conv_input_out_indices", x.indices().numpy())
        np.save(prefix + "conv_input_out_values", x.values().numpy())

        x_conv1 = self.conv1(x)
        np.save(prefix + "x_conv1_out_indices", x_conv1.indices().numpy())
        np.save(prefix + "x_conv1_out_values", x_conv1.values().numpy())

        x_conv2 = self.conv2(x_conv1)
        np.save(prefix + "x_conv2_out_indices", x_conv2.indices().numpy())
        np.save(prefix + "x_conv2_out_values", x_conv2.values().numpy())

        x_conv3 = self.conv3(x_conv2)
        np.save(prefix + "x_conv3_out_indices", x_conv3.indices().numpy())
        np.save(prefix + "x_conv3_out_values", x_conv3.values().numpy())

        x_conv4 = self.conv4(x_conv3)
        np.save(prefix + "x_conv4_out_indices", x_conv4.indices().numpy())
        np.save(prefix + "x_conv4_out_values", x_conv4.values().numpy())

        out = self.extra_conv(x_conv4)
        np.save(prefix + "out_indices", out.indices().numpy())
        np.save(prefix + "out_values", out.values().numpy())
...
def test(cutlass, count):
    paddle.seed(0)
    indices = np.load("./dump_data/x_indices_" + str(count) + ".npy")
    values = np.load("./dump_data/x_values_" + str(count) + ".npy")
    print(indices.shape)
    print(values.shape)
    batch_size = 4
    model = SparseResNet3D(in_channels=5, voxel_size=(0.075, 0.075, 0.2), point_cloud_range=(-54, -54, -5.0, 54, 54, 3.0))
    ...
    out = model(paddle.to_tensor(values), paddle.to_tensor(indices.transpose(1,0)), batch_size)
    paddle.device.cuda.synchronize()

    if cutlass is False:
        #run twice, verify that the params are loaded correctly
        #correct_out = np.load("out.npy")
        #assert np.allclose(out.numpy(), correct_out, rtol=1e-1)

        np.save("out", out.numpy())
    else:
        np.save("out_cutlass", out.numpy())
        correct_out = np.load("out.npy")
        assert np.allclose(out.numpy(), correct_out, rtol=1e-1)
